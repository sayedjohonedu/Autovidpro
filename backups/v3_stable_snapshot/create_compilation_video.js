...
async function renderNumberedRepoSegment({
  ...
  for (let sIdx = 0; sIdx < scriptData.scenes.length; sIdx++) {
    ...
    let actualBeats = [];
    const fallbackGifQuery = scene.fallbackGifQuery || `${repoData.language || 'tech'} cinematic futuristic`;

    const patternIndex = (segmentIndex + sIdx) % 3;
    if (sIdx === 0) {
      if (patternIndex === 0) {
        actualBeats = [
          { type: 'gif_search', query: fallbackGifQuery },
          { type: 'screen_studio_cursor', chunkDuration: 4.0 }
        ];
      } else if (patternIndex === 1) {
        actualBeats = [
          { type: 'screen_studio_cursor', chunkDuration: 4.0 },
          { type: 'github_tv_tilt', label: 'GITHUB REPO TOUR' }
        ];
      } else {
        actualBeats = [
          { type: 'gif_search', query: `${repoData.language || 'tech'} matrix superpower` },
          { type: 'github_tv_tilt', label: 'REPO RADAR' }
        ];
      }
    } else if (sIdx === 1) {
      if (patternIndex === 1) {
        actualBeats = [
          { type: 'screen_studio_cursor', chunkDuration: 4.0 },
          { type: allAuthorMedia.length > 0 ? 'repo_media' : 'github_tv_tilt', label: 'CORE ARCHITECTURE', query: `${repoData.language || 'code'} architecture matrix` }
        ];
      } else {
        actualBeats = [
          { type: 'gif_search', query: `${repoData.language || 'code'} architecture matrix` },
          { type: 'screen_studio_cursor', chunkDuration: 4.0 }
        ];
      }
    } else if (sIdx === 2) {
      actualBeats = [
        { type: 'github_tv_tilt', label: 'DEEP DIVE DEMO' },
        { type: 'screen_studio_cursor', chunkDuration: 3.5 }
      ];
    } else {
      actualBeats = [
        { type: 'screen_studio_cursor', chunkDuration: 3.5 },
        { type: 'gif_search', query: 'stadium crowd cheering laser show fire' }
      ];
    }
...
async function runCompilationProduction(options = {}) {
  const repoCount = options.repoCount || 3; // Default 3 repos for agile generation, scalable to 5-10
  console.log('\n================================================================');
  ...
  const uncoveredList = await trendingScraper.fetchDeepUncoveredTrending(repoCount);
...
if (require.main === module) {
  // Support flexible CLI arguments: e.g. "node create_compilation_video.js 5" or "node create_compilation_video.js 10"
  const cliCount = parseInt(process.argv[2] || process.env.REPO_COUNT || '5', 10);
  const targetCount = isNaN(cliCount) ? 5 : cliCount;
  
  runCompilationProduction({ repoCount: targetCount }).catch(err => {
