...
class CredentialManager {
  ...
  async loadCredentials() {
    try {
      if (process.env.YOUTUBE_CREDENTIALS_JSON) {
        this.credentials = JSON.parse(process.env.YOUTUBE_CREDENTIALS_JSON);
        return;
      }
  ...
  async loadTokens() {
    try {
      if (process.env.YOUTUBE_TOKENS_JSON) {
        this.tokens = JSON.parse(process.env.YOUTUBE_TOKENS_JSON);
        return;
      }
  ...
  async validateAll() {
    ...
    if (missing.length > 0) {
      ...
      console.log(chalk.gray('Any one AI provider is enough — configure .env or run: npm run credentials:setup'));
      return false;
    }

    // Validate YouTube tokens (warning only so local dashboard & generation work)
    if (!this.credentials.youtube || !this.tokens.youtube) {
      console.log(chalk.yellow('⚠️  YouTube OAuth not connected yet. (Video creation & dashboard active; direct auto-upload disabled until authenticated)'));
      console.log(chalk.gray('👉 To connect your YouTube channel later: npm run setup\n'));
    }
