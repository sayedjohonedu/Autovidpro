const axios = require('axios');
const fs = require('fs');
const path = require('path');
const https = require('https');

async function testPerfectWhiteDotGrid() {
  const adcPath = path.join(process.env.HOME || '/Users/sayedjohon', '.config/gcloud/application_default_credentials.json');
  const httpsAgent = new https.Agent({ family: 4, keepAlive: true });
  const creds = JSON.parse(fs.readFileSync(adcPath, 'utf8'));

  const tokenRes = await axios.post('https://oauth2.googleapis.com/token', {
    client_id: creds.client_id,
    client_secret: creds.client_secret,
    refresh_token: creds.refresh_token,
    grant_type: 'refresh_token'
  }, { httpsAgent, timeout: 10000 });
  const token = tokenRes.data.access_token;

  const characterRefPath = path.join(__dirname, 'Assets', 'Character Reference.png');
  const refB64 = fs.readFileSync(characterRefPath).toString('base64');
  const url = 'https://us-central1-aiplatform.googleapis.com/v1/projects/newaug626/locations/us-central1/publishers/google/models/gemini-2.5-flash-image:generateContent';

  const prompt1 = `A viral 16:9 YouTube tech thumbnail in a clean, minimalist high-key white commercial studio setting.
BACKGROUND & ENVIRONMENT: Pure bright off-white studio background covered with a subtle, delicate technical black micro dot-grid pattern. Clean white studio floor with soft realistic ambient contact shadows.
RIGHT SIDE: The female creator from the reference character sheet (exact same face, wavy dark brown hair, warm brown eyes). She fills the right third of the frame from the bottom edge up to her head, showing an expressive shocked mindblown expression (one hand on forehead, wide eyes, looking at camera), seamlessly anchored to the bottom right of the 16:9 frame with soft studio lighting.
CENTER: Large, solid, physical 3D matte-black architectural block letters standing firmly upright on the floor reading "GITHUB AI", casting realistic soft drop shadows and contact shadows onto the white floor.
LEFT SIDE: A heavy brushed stainless-steel circular bank vault door resting on the floor, melting with a vibrant glossy bright lime-green chrome liquid slime dripping into a puddle on the ground.
STYLE & QUALITY: Tangible real-world physical textures, editorial commercial studio photography, crisp 8k resolution, clean composition, zero dark neon void, zero generic cyberpunk circuits, authentic physical materials.`;

  const prompt2 = `A viral 16:9 YouTube tech thumbnail in a clean, minimalist high-key white commercial studio setting.
BACKGROUND & ENVIRONMENT: Pure bright off-white studio background covered with a subtle, delicate technical black micro dot-grid pattern. Clean white studio floor with soft realistic ambient contact shadows.
RIGHT SIDE: The female creator from the reference character sheet (exact same face, wavy dark brown hair, warm brown eyes). She fills the right third of the frame from the bottom edge up, making a secret whisper "shhh" gesture with one finger on her lips, looking directly at the camera with an intriguing smirk.
CENTER: Large, solid, physical 3D matte-black architectural block letters standing firmly upright on the floor reading "SECRET REPO", casting realistic soft drop shadows onto the white floor.
LEFT SIDE: A tactile matte green mechanical hardware UI toggle switch melting with thick glossy white candle wax dripping into a puddle on the floor.
STYLE & QUALITY: Tangible real-world physical textures, editorial commercial studio photography, crisp 8k resolution, clean composition, authentic physical materials.`;

  console.log('Generating Perfect White Dot-Grid Variant 1...');
  const res1 = await axios.post(url, {
    contents: [{
      role: 'user',
      parts: [
        { text: prompt1 },
        { inlineData: { mimeType: 'image/png', data: refB64 } }
      ]
    }],
    generationConfig: { responseModalities: ['IMAGE', 'TEXT'] }
  }, { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, httpsAgent, timeout: 60000 });

  const p1 = res1.data?.candidates?.[0]?.content?.parts?.find(p => p.inlineData?.data);
  if (p1) {
    const out1 = path.join(__dirname, 'temp', 'white_dotgrid_vault_shocked.png');
    fs.writeFileSync(out1, Buffer.from(p1.inlineData.data, 'base64'));
    console.log(`✅ Saved Variant 1: ${out1}`);
  }

  console.log('Generating Perfect White Dot-Grid Variant 2 (Melting Toggle Switch)...');
  const res2 = await axios.post(url, {
    contents: [{
      role: 'user',
      parts: [
        { text: prompt2 },
        { inlineData: { mimeType: 'image/png', data: refB64 } }
      ]
    }],
    generationConfig: { responseModalities: ['IMAGE', 'TEXT'] }
  }, { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, httpsAgent, timeout: 60000 });

  const p2 = res2.data?.candidates?.[0]?.content?.parts?.find(p => p.inlineData?.data);
  if (p2) {
    const out2 = path.join(__dirname, 'temp', 'white_dotgrid_toggle_secret.png');
    fs.writeFileSync(out2, Buffer.from(p2.inlineData.data, 'base64'));
    console.log(`✅ Saved Variant 2: ${out2}`);
  }
}

testPerfectWhiteDotGrid().catch(console.error);
